...
import { resolveResponseExamples } from "./util/resolveResponseExamples.js";
...
export const Sidecar = ({
  ...
  const hasAnyResponseExamples = operation.responses.some((response) =>
    response.content?.some((content) => (content.examples?.length ?? 0) > 0),
  );

  const resolvedResponses = resolveResponseExamples(operation.responses).map(
    (response) => ({
      ...response,
      content:
  ...
  return (
    ...
    >
      ...
      <ResponsesSidecarBox
        isGenerated={!hasAnyResponseExamples}
        isOnScreen={isOnScreen}
        shouldLazyHighlight={shouldLazyHighlight}
