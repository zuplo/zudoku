...
const BaseConfigSchema = z.object({
  /**
   * Base config layers this config composes, as module specifiers
   * (tsconfig-style, resolved lazily relative to this config file — e.g. a
   * config generated by `zudoku generate` just before dev/build) or inline
   * config objects. Layers are merged in order, with this config applied on
   * top: scalars and objects from later layers win, `plugins` concatenate.
   */
  // Typed as `(string | ZudokuConfig)[]` via `InputOverrides` below;
  // `z.custom` can't reference the config type itself without a circular
  // type error.
  extends: z.array(z.union([z.string(), z.custom<Record<string, unknown>>()])),
