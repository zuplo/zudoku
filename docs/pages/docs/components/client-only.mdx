...
A component that only renders its children on the client-side, preventing
...
- **Browser APIs**: Components that use `window`, `document`, or other
  browser-only APIs
...
Use ClientOnly sparingly as it can impact SEO and initial page load performance.
Only use it when necessary for components that cannot be server-rendered.

:::

:::tip

Always provide a meaningful fallback that matches the approximate size and
layout of your client-only component to prevent layout shifts.
