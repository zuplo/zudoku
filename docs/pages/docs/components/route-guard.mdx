...
```tsx
function CustomRouteGuard({ children, customCheck, fallback = <AccessDenied /> }) {
  const { user } = useAuth();
...
function TimeBasedGuard({ children, allowedHours }) {
  ...
  return <RouteGuard>{isAllowed ? children : <OutsideBusinessHours />}</RouteGuard>;
}
```
...
function SubscriptionGuard({ children, requiredPlan }) {
  ...
  return <RouteGuard>{hasValidSubscription ? children : <UpgradePrompt />}</RouteGuard>;
}
```
...
:::tip Use RouteGuard at multiple levels - route level for page protection and component level for
feature protection. :::

:::info RouteGuard integrates with Zudoku's authentication system and will automatically check user
authentication state and permissions. :::

:::caution Always provide meaningful fallback content to guide users on how to gain access to
protected content. :::
