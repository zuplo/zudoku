...
For external links, use a regular `<a>` tag instead of the Link component to ensure proper external
navigation behavior.
...
When using `reloadDocument`, the link will perform a full page reload, which may impact performance
and user experience.
