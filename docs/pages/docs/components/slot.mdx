...
description:
  Use the low-level Slot component for advanced slot management in custom pages
  and MDX content.
---

The `Slot` component provides low-level access to Zudoku's slot system, allowing
...
registers content to be displayed by `Slot.Target` components.
...
The Slot component is fully type-safe. All predefined slot names are available
with autocomplete:
...
`zudoku.config.tsx` and content injected via `Slot.Source` components will be
combined according to the `type` parameter.
...
For basic configuration usage, see the
[Slots Configuration](/docs/configuration/slots) documentation.
