...
A config can extend one or more base configs via the `extends` option. This is useful when part of
your configuration is generated by a tool (e.g. `zudoku generate`, or the Zuplo integration) and you
want to keep your own settings separate from it.
...
Layers are merged in order with the extending config applied last, so it wins on conflicts. Nested
objects are merged recursively, while `plugins` and `apis` entries are concatenated (base layers
first). All other values are replaced by the topmost layer that sets them.

### Generating a base config

The `zudoku generate` command compiles a serializable spec file into a typed base config layer:

```bash
zudoku generate spec.json -o zudoku.base.ts
```

The spec file is plain JSON validated against a published JSON Schema (print it with
`zudoku generate --print-schema`). Plugins are referenced by name in the spec and turned into real
static imports in the generated file, so the generated layer builds and renders like any
hand-written config. When a `spec.json` is present (or the project runs in Zuplo mode), the generate
step runs automatically before `zudoku dev`, `zudoku build` and `zudoku preview`.
