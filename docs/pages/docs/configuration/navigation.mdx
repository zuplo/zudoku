...
Add an icon to any category or document with the `icon` property. The value is an
[Iconify](https://iconify.design) `prefix:name` string, for example `lucide:book` <Icon
icon="lucide:book" width={16} style={{ display: 'inline' }} />, `lucide:code` <Icon
icon="lucide:code" width={16} style={{ display: 'inline' }} />, or `lucide:file-text` <Icon
icon="lucide:file-text" width={16} style={{ display: 'inline' }} />.

The default prefix is `lucide`, so a bare name like `"book"` resolves to `"lucide:book"`. To use
other icon sets (e.g. `ph:`, `mdi:`), install the matching `@iconify-json/*` package. See the
[Icons reference](/docs/components/icons) for details.
