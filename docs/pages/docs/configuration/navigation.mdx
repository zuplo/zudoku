...
description:
  Learn how to configure the navigation in Zudoku, including links, categories, documents, custom
  pages, separators, sections, and filters. Understand the structure of the navigation array and how
  to use icons, labels, and paths.
---
...
- `separator`: A horizontal line to visually separate groups of items.
- `section`: A label/header to group items without nesting them in a category.
- `filter`: A search input that filters navigation items by their label.
...
A separator renders a horizontal line in the sidebar to visually divide groups of navigation items.
...
A section displays a label/header that groups items without nesting them in a collapsible category.
Useful for organizing the sidebar into logical groups while keeping items flat.
...
A filter adds a search input to the sidebar that filters navigation items by their label. When users
type in the filter, only matching items are displayed.
...
### Using sections, separators, and filters

Here's a complete example showing how these items work together:
...
This can be a preset string or a custom function for more complex logic.
...
For more complex visibility logic, you can provide a function that receives the current context and
...
```tsx
{
  ...
  label: "Beta Features",
  to: "/beta",
  display: ({ context, auth }) => {
    // Show only to authenticated users with beta access
    return auth.isAuthenticated && auth.profile?.betaAccess === true;
  }
}
```
...
- `auth`: The authentication state from `UseAuthReturn`
