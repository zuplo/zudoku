...
- `separator`: A visual separator line between navigation items.
- `section`: A non-collapsible label/heading in the sidebar.
- `filter`: Adds a filter/search input to the sidebar for quick filtering of navigation items.
...
type NavigationCustomPage = {
  ...
  element: any;
  icon?: string; // Lucide icon name
  layout?: "default" | "none"; // Page layout mode
  badge?: {
    label: string;
    color: "green" | "blue" | "yellow" | "red" | "purple" | "indigo" | "gray" | "outline";
    invert?: boolean;
...
A visual separator line between navigation items. Useful for visually grouping items in the sidebar.
...
A non-collapsible label/heading in the sidebar. Unlike categories, sections don't wrap child items -
they simply display a label.
...
Adds a filter/search input to the sidebar that allows users to quickly filter navigation items. You
can customize the placeholder text.
