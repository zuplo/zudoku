...
Plugins automatically generate sidebar navigation (e.g. from OpenAPI tags). Navigation rules let you
customize that generated sidebar by inserting, modifying, or removing items. To change the top-level
tabs themselves, use the `navigation` array directly.

For a practical walkthrough with examples, see the [Navigation Rules](/docs/guides/navigation-rules)
guide.
