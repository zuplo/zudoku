...
1. **Change the Basics**
   ...
   **Have a company logo?** Learn how you can
   [add your logo](https://zuplo.com/docs/dev-portal/zudoku/configuration/page#logo) in our
   documentation.
...
1. **API Reference**
   ...
   Your [API Reference](/api) is generated automatically from the OpenAPI files in your Zuplo
   project's `config` directory via the generated `zudoku-zuplo.config.ts`, which your config
   includes through the `extends` option. Improve your documentation by adding more details to the
   OpenAPI file.
   ...
   Prefer to configure your APIs yourself? Remove the `extends` entry and add an
   [`apis`](https://zuplo.com/docs/dev-portal/zudoku/configuration/api-reference) entry instead.
